...
struct FkResponse {
    ...
    end_effector_orientation: [f64; 4],
...
struct TrajectoryResponse {
    ...
    optimized_waypoints: Vec<TrajectoryPoint>,
    total_distance: f64,
    total_time: f64,
    max_velocity_reached: f64,
    elapsed_us: u128,
}
#[derive(Serialize)]
...
async fn optimize_trajectory(
    ...
    for i in 0..waypoints.len() {
        ...
        let seg_time = if seg_dist > 0.0 {
            seg_dist / (max_vel * 0.8)
        } else {
            0.0
        };
        cumulative_time += seg_time;

        let vel_mag = if seg_time > 0.0 {
            seg_dist / seg_time
        } else {
            0.0
        };
        if vel_mag > max_vel_reached {
            max_vel_reached = vel_mag;
        }
        ...
        optimized.push(TrajectoryPoint {
            position: pos,
            velocity,
    ...
    Json(TrajectoryResponse {
        ...
        max_velocity_reached: max_vel_reached,
        elapsed_us: t.elapsed().as_micros(),
    })
}
...
async fn chains() -> Json<Vec<ChainInfo>> {
    Json(vec![
        ChainInfo {
            id: "human_arm".into(),
            name: "Human Arm".into(),
            description: "7-DOF human arm: shoulder(3) + elbow(1) + wrist(3)".into(),
            dof: 7,
        ...
        ChainInfo {
            id: "human_leg".into(),
            name: "Human Leg".into(),
            description: "6-DOF human leg: hip(3) + knee(1) + ankle(2)".into(),
            dof: 6,
            joint_type: "revolute".into(),
        },
        ChainInfo {
            id: "robotic_arm_6dof".into(),
            name: "Robotic Arm (6-DOF)".into(),
            description: "Standard industrial 6-DOF manipulator".into(),
            dof: 6,
            joint_type: "revolute".into(),
        },
        ChainInfo {
            id: "delta_robot".into(),
            name: "Delta Robot".into(),
        ...
        ChainInfo {
            id: "scara".into(),
            name: "SCARA".into(),
            description: "4-DOF selective compliance assembly robot arm".into(),
            dof: 4,
            joint_type: "revolute+prismatic".into(),
        },
    ])
}
...
async fn stats(State(s): State<Arc<AppState>>) -> Json<StatsResponse> {
    let st = s.stats.lock().unwrap();
    Json(StatsResponse {
        total_ik_solves: st.total_ik_solves,
        total_fk_solves: st.total_fk_solves,
        total_compressions: st.total_compressions,
        total_trajectories: st.total_trajectories,
    })
}
